...
class KMeans(Model):
    ...
    def __init__(self, k, n_proc=2, distance_metric='euclidean',
    ...
    def _clust_init(self, x):
        # change to get centers from chunks
        inx = np.random.choice(range(x.shape[0]), self.k, replace=False)
        self.clust_centers = x[inx]

    def _proc_run(self, chunk):
        chunk = np.array(chunk)
        C = np.array([np.argmin([np.dot(x_i - y_k, x_i - y_k) for y_k in self.clust_centers]) for x_i in chunk])
        centroids = []
    ...
    def fit(self, x):
        ...
        for i in range(self.n_iter):
            ...
            self.clust_centers = np.array(np.nanmean(arrays, axis=0).tolist())
        self.labels = []
        for i in x:
            tmp = []
            for y in self.clust_centers:
                tmp += [np.dot(i - y, i - y)]
